...
const DEFAULT_SMTP_HOST: &str = "smtp.office365.com";
const DEFAULT_SMTP_PORT: u16 = 587;
const DEFAULT_IMAP_PASSWORD_ENV: &str = "OUTLOOK_EMAIL_PASSWORD";
const DEFAULT_SMTP_PASSWORD_ENV: &str = "OUTLOOK_EMAIL_PASSWORD";
...
pub fn ensure_configured(config: &ChannelConfig) -> Result<()> {
    if !has_imap_config(config) && !has_smtp_config(config) {
        bail!("outlook plugin requires IMAP ingress, SMTP delivery, or both");
    }
