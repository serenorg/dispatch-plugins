...
const PLATFORM_EMAIL: &str = "outlook";
...
fn handle_request(
    ...
    match &envelope.request {
        ...
        PluginRequest::IngressEvent { .. } => Ok(plugin_error(
            ...
            "outlook ingress is IMAP-backed and does not accept webhook payloads",
        )),
        PluginRequest::Status { .. } => Ok(PluginResponse::StatusAccepted {
            status: rejected_status("not_supported", "outlook status frames are not supported"),
        }),
            ...
            match poll_ingress(&config, prior_state) {
                Ok(PluginResponse::IngressEventsReceived {
                    ...
                    if let Err(error) = emit_channel_event_notification(
                        ...
                        eprintln!("outlook ingress worker failed to emit notification: {error}");
                        break;
                    }
                }
                Ok(PluginResponse::Error { error }) => {
                    eprintln!(
                        "outlook ingress worker stopped after plugin error {}: {}",
                ...
                Ok(other) => {
                    eprintln!(
                        "outlook ingress worker received unexpected response variant: {:?}",
                        other
                    );
                    break;
                }
                Err(error) => {
                    eprintln!("outlook ingress worker stopped after receive failure: {error}");
                    break;
                }
            }
...
fn resolve_outgoing_email(
    ...
    for attachment in &message.attachments {
        let data_base64 = attachment.data_base64.clone().ok_or_else(|| {
            anyhow!(
                "outlook outbound attachments require data_base64 for attachment `{}`",
                attachment.name
            )
        })?;
...
"#;
    ...
    fn build_inbound_event_skips_self_authored_sender() {
        let raw = br#"From: Dispatch Bot <bot@outlook.com>
Date: Sat, 20 Nov 2021 14:22:01 -0800
Message-ID: <msg-1@example.com>
Subject: Loop
Content-Type: text/plain; charset="utf-8"
...
"#;
        ...
        let event = build_inbound_event(
            &ChannelConfig {
                imap_username: Some("bot@outlook.com".to_string()),
                smtp_from_email: Some("bot@outlook.com".to_string()),
                ..ChannelConfig::default()
            },
            &MailboxStatus {
                mailbox: "INBOX".to_string(),
    ...
    fn resolve_outgoing_email_prefers_routing_metadata() {
        let config = ChannelConfig {
            smtp_username: Some("bot@outlook.com".to_string()),
            smtp_password_env: Some("OUTLOOK_EMAIL_PASSWORD".to_string()),
            smtp_from_email: Some("bot@outlook.com".to_string()),
            default_recipient: Some("fallback@outlook.com".to_string()),
    ...
    fn configure_minimal_preset_uses_outlook_defaults() {
        ...
        let configured = configure(&config).expect("minimal outlook config should configure");
