...
impl SlackClient {
    ...
    /// Fetch exactly one already-delivered message by its Slack `ts`.
    ///
    /// Uses the narrowest read for the shape of the message: `conversations.replies`
    /// bounded to the single `ts` when a distinct thread parent is supplied,
    /// otherwise `conversations.history` bounded to the single `ts`. Returns
    /// `Ok(None)` when the message is missing or deleted — never a different
    /// nearby message. The bot token is workspace-scoped, so a channel in another
    /// workspace fails the underlying Slack call rather than resolving here.
    pub fn fetch_message(
        ...
        if !body.get("ok").and_then(Value::as_bool).unwrap_or(false) {
            ...
            if matches!(
                error,
                "messages_not_found" | "message_not_found" | "thread_not_found"
            ) {
                return Ok(None);
            }
        ...
        Ok(Some(SlackFetchedMessage {
            ...
            thread_id: message
                ...
                .and_then(Value::as_str)
                .map(ToOwned::to_owned)
                .or_else(|| thread_ts.map(ToOwned::to_owned)),
    ...
    /// Resolve the canonical permalink for one exact message.
    ///
    /// Returns `Ok(None)` when the message no longer exists so callers can report
    /// a stable not-found rather than a spurious error.
    pub fn message_permalink(&self, channel_id: &str, message_ts: &str) -> Result<Option<String>> {
        ...
        if !body.get("ok").and_then(Value::as_bool).unwrap_or(false) {
            let error = body
                ...
                .and_then(Value::as_str)
                .unwrap_or("unknown_slack_error");
            if matches!(error, "message_not_found" | "channel_not_found") {
                return Ok(None);
            }
...
mod tests {
    ...
    /// Serve one JSON response and capture the single request the client sends.
    fn serve_once(
        body: &'static str,
    ) -> (
        String,
    ...
    fn fetch_message_reads_single_history_message() {
        ...
        assert_eq!(fetched.workspace_id.as_deref(), Some("T1"));
