...
use imap_smtp::{
    ...
    ensure_configured, fetch_messages_since, has_imap_config, has_smtp_config, imap_host,
    imap_mailbox, imap_password_env, imap_port, latest_existing_uid, require_imap_config,
    send_email, smtp_connection_ok, smtp_from_email, smtp_host, smtp_password_env, smtp_port,
};
use protocol::{
...
const PLATFORM_EMAIL: &str = "gmail";
...
fn handle_request(
    ...
    match &envelope.request {
        ...
        PluginRequest::IngressEvent { .. } => Ok(plugin_error(
            ...
            "gmail ingress is IMAP-backed and does not accept webhook payloads",
        )),
        PluginRequest::Status { .. } => Ok(PluginResponse::StatusAccepted {
            status: rejected_status("not_supported", "gmail status frames are not supported"),
        }),
            ...
            match poll_ingress(&config, prior_state) {
                Ok(PluginResponse::IngressEventsReceived {
                    ...
                    if let Err(error) = emit_channel_event_notification(
                        ...
                        eprintln!("gmail ingress worker failed to emit notification: {error}");
                        break;
                    }
                }
                Ok(PluginResponse::Error { error }) => {
                    eprintln!(
                        "gmail ingress worker stopped after plugin error {}: {}",
                ...
                Ok(other) => {
                    eprintln!(
                        "gmail ingress worker received unexpected response variant: {:?}",
                        other
                    );
                    break;
                }
                Err(error) => {
                    eprintln!("gmail ingress worker stopped after receive failure: {error}");
                    break;
                }
            }
...
fn configure(config: &ChannelConfig) -> Result<ConfiguredChannel> {
    ...
    if has_imap_config(config) {
        metadata.insert(META_IMAP_HOST.to_string(), imap_host(config)?);
        metadata.insert(META_IMAP_PORT.to_string(), imap_port(config).to_string());
        metadata.insert(
    ...
    if has_smtp_config(config) {
        metadata.insert(META_SMTP_HOST.to_string(), smtp_host(config)?);
        metadata.insert(META_SMTP_PORT.to_string(), smtp_port(config).to_string());
        metadata.insert(
...
fn resolve_outgoing_email(
    ...
    for attachment in &message.attachments {
        let data_base64 = attachment.data_base64.clone().ok_or_else(|| {
            anyhow!(
                "gmail outbound attachments require data_base64 for attachment `{}`",
                attachment.name
            )
        })?;
...
"#;
    ...
    fn build_inbound_event_skips_self_authored_sender() {
        let raw = br#"From: Dispatch Bot <bot@gmail.com>
Date: Sat, 20 Nov 2021 14:22:01 -0800
Message-ID: <msg-1@example.com>
Subject: Loop
Content-Type: text/plain; charset="utf-8"
...
"#;
        ...
        let event = build_inbound_event(
            &ChannelConfig {
                imap_username: Some("bot@gmail.com".to_string()),
                smtp_from_email: Some("bot@gmail.com".to_string()),
                ..ChannelConfig::default()
            },
            &MailboxStatus {
                mailbox: "INBOX".to_string(),
    ...
    fn resolve_outgoing_email_prefers_routing_metadata() {
        let config = ChannelConfig {
            smtp_username: Some("bot@gmail.com".to_string()),
            smtp_password_env: Some("GMAIL_APP_PASSWORD".to_string()),
            smtp_from_email: Some("bot@gmail.com".to_string()),
            default_recipient: Some("fallback@gmail.com".to_string()),
    ...
    fn configure_minimal_preset_uses_gmail_defaults() {
        // Safety: test mutates the process env. Only gmail tests read this.
        // SAFETY: set_var is unsafe because it mutates shared process state;
        // these tests do not rely on env vars concurrently.
        unsafe {
        ...
        let configured = configure(&config).expect("minimal gmail config should configure");

        assert_eq!(
            configured.metadata.get(META_IMAP_HOST).map(String::as_str),
            Some("imap.gmail.com")
        );
        assert_eq!(
            configured.metadata.get(META_IMAP_PORT).map(String::as_str),
            Some("993")
        );
        assert_eq!(
            configured.metadata.get(META_SMTP_HOST).map(String::as_str),
            Some("smtp.gmail.com")
        );
        assert_eq!(
            configured.metadata.get(META_SMTP_PORT).map(String::as_str),
            Some("465")
        );
        assert_eq!(
            configured
                .metadata
                .get(META_IMAP_PASSWORD_ENV)
                .map(String::as_str),
