...
A gateway `MESSAGE_CREATE` carries no channel object. An explicitly allow-listed channel is placed from the guild id already validated against `allowed_guild_ids`, so an authorized direct mention is never lost when `GET /channels/{id}` is unavailable. A channel the API reports as a thread still answers to thread policy; only exact channel-id matches take that path, so authorization stays fail-closed.

### Ingress observability

Ingress state carries always-on, content-free counters and freshness timestamps that ride every notification, independent of any debug flag: dispatch frames by type (`frames_dispatch_total`, `frames_message_create`, `frames_ready`, `frames_other_dispatch`), decoded and accepted messages (`messages_decoded`, `messages_accepted`), emitted notifications (`notifications_emitted`), decode failures (`message_decode_errors`), and per-reason rejection counts keyed `reject_<reason>` over the stable vocabulary: `bot_event`, `unsupported_message_type`, `unauthorized_workspace`, `unauthorized_channel`, `unauthorized_thread`, `unresolved_conversation`, `dm_denied`, `sender_denied`, `not_addressed`, `empty_message`, `application_mismatch`, `invalid_policy`, and `notification_write_error`. Timestamps report `last_dispatch_frame_at`, `last_message_frame_at`, `last_heartbeat_at`, and `last_notification_at`. `event_path_ready` reports the actionable message-to-notification path separately from heartbeat and dispatch-frame freshness: a decode or emission failure degrades it, and a fresh heartbeat ACK never clears a known failure. Counters are carried across reconnects through ingress state. Set `DISCORD_GATEWAY_DEBUG=1` to additionally log each drop with its provider message id.
