...
Typing uses the same destination resolution and authorization as delivery, so a destination outside the outbound scope produces no typing request at all. Each destination has at most one worker and repeated starts join it rather than opening a second one. Concurrent turns in one destination therefore share a single indicator, and the first terminal frame for that destination stops it, because status frames carry no turn identifier; a later processing frame starts a fresh worker. The indicator is refreshed every eight seconds, ahead of Discord's ten-second expiry, and a worker expires after five minutes so a producer that never sends a terminal frame cannot leave one running. A worker survives an isolated failed refresh and stops after three consecutive failures.

Workers are capped at 64 destinations. Beyond that a processing frame is accepted and reports `at_capacity`, and no indicator is shown for that destination until a slot frees. Successful delivery, configuration replacement, ingress stop, shutdown, and process exit stop the relevant workers; each of those waits for an in-flight typing request, which is bounded by a five-second request timeout. Discord has no stop-typing endpoint, so the last indicator fades on its own expiry rather than being cleared.

Typing failures do not fail the managed turn, and delivery is unaffected. An initial typing failure is reported in status metadata; later refresh failures appear in health metadata. Status metadata reports `typing_state` as `started`, `active`, `at_capacity`, `failed`, `stopped`, or `inactive`. Health metadata reports content-free counts for starts, refreshes, stops, failures, and capacity refusals, plus UTC timestamps for the last successful request and failure, with no destinations, message content, or provider bodies.
